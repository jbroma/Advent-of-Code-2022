...
export function pickBestNode(
    ...
    const reachableNodes = nodes.slice().filter(([_, c]) => c < movesLeft);
    const maxCost = reachableNodes.sort((a, b) => b[1] - a[1])[0]?.[1];
    if (maxCost === undefined) return null;
    const potentialValues = reachableNodes.map(([node, cost]) => [
        node,
...
function findBestPath(
    ...
    if (!bestTarget) {
        const ntvc = new Set(nodesToVisit);
        ntvc.clear();
        return findBestPath(node, ntvc, pressure, movesLeft, flowRate);
    }
    return findBestPath(
        bestTarget[0],
        nodesToVisit,
...
export default function () {
    ...
    lineReader.on('close', () => {
        ...
        let max = 0;
        while (true) {
            const commonValves = new Set(nodesWithFlow);
            const result1 = findBestPath(startingNode, commonValves, 0, 26, 0);
            const result2 = findBestPath(startingNode, commonValves, 0, 26, 0);
