...
function isValidMove(currentHeight: string, targetHeight: string): boolean {
    const cached = isValidMoveMap.get(currentHeight + targetHeight);
    if (cached) return cached;
    const adjustedCurrentHeight = currentHeight === 'E' ? 'z' : currentHeight;
    const adjustedTargetHeight = targetHeight === 'S' ? 'a' : targetHeight;
    const result =
